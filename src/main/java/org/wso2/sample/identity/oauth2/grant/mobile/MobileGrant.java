    ...
    public boolean validateGrant(OAuthTokenReqMessageContext oAuthTokenReqMessageContext)  throws IdentityOAuth2Exception {
        ...
        if(mobileNumber != null) {
            ...
            try {
                ...
                throw new IdentityOAuth2Exception("Error while authenticating", e);
            }
            ...
            if(authStatus) {
                ...
                oAuthTokenReqMessageContext.addProperty("custom_access_token", "cat-" + UUID
                        .randomUUID().toString());
                oAuthTokenReqMessageContext.addProperty("custom_refresh_token", "crt-" + UUID
                        .randomUUID().toString());
                oAuthTokenReqMessageContext.addProperty("custom_validity_period", "1000");

                /**
                 RequestParameter[] requestParameters = Arrays.copyOf(parameters, parameters.length + 2);

                 RequestParameter accessToken = new RequestParameter("custom_access_token", "cat-" + UUID
                 .randomUUID().toString());
                 RequestParameter refreshToken = new RequestParameter("custom_refresh_token", "crt-" + UUID
                 .randomUUID().toString());

                 requestParameters[requestParameters.length - 2] = accessToken;
                 requestParameters[requestParameters.length - 1] = refreshToken;
...
//        return authzCallback.isAuthorized();
    ...
    protected void storeAccessToken(OAuth2AccessTokenReqDTO oAuth2AccessTokenReqDTO, String userStoreDomain,
                                    AccessTokenDO newAccessTokenDO, String newAccessToken,
                                    AccessTokenDO existingAccessTokenDO) throws IdentityOAuth2Exception {

        /**
         RequestParameter[] parameters = oAuth2AccessTokenReqDTO.getRequestParameters();

         RequestParameter[] requestParameters = Arrays.copyOf(parameters, parameters.length + 1);

         RequestParameter isNewToken = new RequestParameter("is_new_token", "true");
         requestParameters[requestParameters.length - 1] = isNewToken;

         newAccessTokenDO.setAccessToken(parameters[2].getValue()[0]);
         newAccessTokenDO.setRefreshToken(parameters[3].getValue()[0]);

         newAccessToken = String.valueOf(parameters[2].getValue()[0]);

         **/

        super.storeAccessToken(oAuth2AccessTokenReqDTO, userStoreDomain, newAccessTokenDO, newAccessToken,
    ...
    public boolean validateScope(OAuthTokenReqMessageContext tokReqMsgCtx)
        ...
        if (super.validateScope(tokReqMsgCtx)) {
            ...
            tokReqMsgCtx.setValidityPeriod(Long.parseLong(String.valueOf(tokReqMsgCtx.getProperty("custom_validity_period"))));
            return true;
        }
...
//        return scopeValidationCallback.isValidScope();
    ...
    public OAuth2AccessTokenRespDTO issue(OAuthTokenReqMessageContext tokReqMsgCtx) throws IdentityOAuth2Exception {
        ...
        /**
         ...
         if (Boolean.parseBoolean(parameters[4].getValue()[0])) {

         oAuth2AccessTokenRespDTO.setAccessToken(parameters[2].getValue()[0]);
         oAuth2AccessTokenRespDTO.setRefreshToken(parameters[3].getValue()[0]);
     ...
     * @return
     */
    private boolean isValidMobileNumber(String mobileNumber) throws IdentityOAuth2Exception {

        // just demo validation

        if(mobileNumber.startsWith("033")) {

            PostMethod postMethod = new PostMethod("https://httpbin.org/post?q=w");
            HttpClient httpClient = new HttpClient();
            try {
                httpClient.executeMethod(postMethod);
                Map<String, Object> stringObjectMap = JSONUtils.parseJSON(postMethod.getResponseBodyAsString());

                log.info(stringObjectMap);
            } catch (IOException e) {
                log.error("Error sending the post", e);
            }

            return true;
        } else {
            throw new IdentityOAuth2Exception("Invalid mobile number");
        }
